...
export default function Chatbot() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: '**Error**: Sorry, I failed to process your request. Please try again later.' },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-[#0d1117] text-[#c9d1d9] font-sans selection:bg-[#388bfd]/30">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 w-full max-w-4xl mx-auto scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center p-8 mt-12 mb-8 animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-[#8b949e] text-center max-w-md leading-relaxed text-sm">
              I&apos;m your technical mentor. Ask me about a problem, and let&apos;s discover the minimal time complexity approach together.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center w-full max-w-md">
               <button 
                  ...
                  <span className="text-[#8b949e] mr-2 text-xs">$</span> Two Sum optimally?
               </button>
               <button 
                  className="px-4 py-2.5 bg-[#21262d] border border-[#30363d] rounded-md text-sm text-[#c9d1d9] hover:bg-[#30363d] hover:border-[#8b949e] transition-colors text-left flex-1 font-mono hover:text-white" 
                  onClick={() => setInput("What's the intuition behind sliding window?")}>
                 <span className="text-[#8b949e] mr-2 text-xs">$</span> Sliding window intuition?
               </button>
            </div>
          </div>
        ) : (
          <div className="space-y-6 pb-24 fade-in duration-200">
            {messages.map((m, index) => (
              ...
              >
                <div
                  className={`flex flex-col max-w-full sm:max-w-[85%] border rounded-lg overflow-hidden ${
                    m.role === 'user'
                ...
                >
                   ...
                   {m.role === 'user' && (
                       <div className="px-4 py-2 bg-[#21262d] border-b border-[#30363d] flex items-center gap-2 text-xs font-mono text-[#8b949e]">
      ...
      <div className="p-4 sm:p-6 bg-[#0d1117] sticky bottom-0 border-t border-[#30363d]">
        <div className="max-w-4xl mx-auto relative group">
          ...
          >
            ...
            <textarea
              className="flex-1 bg-transparent border-none text-[#e6edf3] placeholder-[#8b949e] px-3 py-3.5 focus:outline-none focus:ring-0 resize-none max-h-48 min-h-12.5 font-mono text-sm"
              ...
              onChange={(e) => {
                ...
                e.target.style.height = Math.min(e.target.scrollHeight, 192) + 'px'; // Max 48*4
              }}
              onKeyDown={(e) => {
          ...
          <div className="flex justify-between items-center mt-2 px-1 text-[11px] text-[#8b949e] font-sans">
             ...
             <span className="hidden sm:inline">Press <kbd className="font-mono bg-[#161b22] border border-[#30363d] rounded px-1.5 py-0.5 mx-0.5 text-[#c9d1d9] font-semibold">Enter</kbd> ↵ to send</span>
          </div>
        </div>
      </div>
    </div>
